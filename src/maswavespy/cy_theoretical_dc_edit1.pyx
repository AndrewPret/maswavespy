...
def compute_fdma(np.ndarray[DTYPE_t, ndim=1] c_test, double c_step, np.ndarray[DTYPE_t, ndim=1] wavelength,
    ...
    for i in range(wavelength_len):
        ...
        if i == 0:
            # First point: broad search in [c_min, c_max]
            c_start = c_test[0]
            c_end   = c_test[-1]
        else:
            # Initial narrow search around previous solution
            c_start = max(c_test[0], c_t[i-1] - delta_c)
            c_end   = min(c_test[-1], c_t[i-1] + delta_c)

        found_root = False
        expand_factor = 2   # how much to expand each time
        max_expand    = 50.0   # don’t expand more than ± max_expand × delta_c

        while not found_root and (c_end - c_start) <= max_expand * 2 * delta_c:
            # Step size for coarse scan (fixed number of sub-intervals)
            c_step_local = (c_end - c_start) / 50

            c_low = c_start
            f_low = dispersion_function(c_low, ki, n, alpha, beta, rho, h, alpha2, beta2)

            for j in range(1, 51):
                c_high = c_start + j * c_step_local
                f_high = dispersion_function(c_high, ki, n, alpha, beta, rho, h, alpha2, beta2)

                if f_low * f_high < 0:   # found a bracket
                    c_t[i] = bisection_root(c_low, c_high, ki, n,
                                            alpha, beta, rho, h,
                                            alpha2, beta2, tol=1e-4)
                    found_root = True
                    break
            ...
            if not found_root:
                ...
                c_start = max(c_test[0], c_t[i-1] - (c_end - c_start) * expand_factor / 2.0)
                c_end   = min(c_test[-1], c_t[i-1] + (c_end - c_start) * expand_factor / 2.0)

        if not found_root:
            # fallback: assign previous value or NaN if no root found
            c_t[i] = c_t[i-1] if i > 0 else np.nan
...
cdef double dispersion_function(double c, double ki, int n,
                                double[:] alpha, double[:] beta,
                                double[:] rho, double[:] h,
                                double[:] alpha2, double[:] beta2):
    """
    Compute the Rayleigh-wave dispersion function at trial velocity c.
    Returns a signed double (not just sign).
    """
    cdef double c2, G02, eta, a, ak, b, bk, D
    cdef double complex X0, X1, X2, X3, X4, r, s, krh, ksh, C_alpha, C_beta, S_alpha, S_beta, p1, p2, p3, p4, q1, q2, y1, y2, z1, z2
    ...
    # Layer recursion
    for q in range(n):
        eta = 2 / c2 * (beta2[q] - (rho[q+1]/rho[q]) * beta2[q+1])
        a = (rho[q+1]/rho[q]) + eta
        ak = a - 1
        b = 1 - eta
        bk = b - 1
        ...
        X0 = bk * y1 + b * y2
        X1 = a * y1 + ak * y2
        X2 = (rho[q+1]/rho[q]) * (C_alpha * p3 - r * S_alpha * p4)
        X3 = (rho[q+1]/rho[q]) * (-(1 / r) * S_alpha * p1 + C_alpha * p2)
        X4 = bk * z1 + b * z2

    r = sqrt(max(1 - c2/alpha2[q], 1e-12))
    s = sqrt(max(1 - c2/beta2[q], 1e-12))
    D = (X1 + s * X2 - r * (X3 + s * X4)).real  # take real part

    return D

cdef double bisection_root(double c_low, double c_high, double ki, int n,
                           double[:] alpha, double[:] beta, double[:] rho, double[:] h,
                           double[:] alpha2, double[:] beta2, double tol=1e-4):
    """
    Refine root of dispersion function between c_low and c_high using bisection.
    """
    cdef double f_low, f_high, f_mid, c_mid
    cdef int it
    ...
    if f_low * f_high > 0:
        return 0.5 * (c_low + c_high)  # fallback

    for it in range(50):  # max iterations
        c_mid = 0.5 * (c_low + c_high)
        f_mid = dispersion_function(c_mid, ki, n, alpha, beta, rho, h, alpha2, beta2)

        if fabs(f_mid) < tol or (c_high - c_low) < tol:
            return c_mid
